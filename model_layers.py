...
def sqnet_expand(x, mode, params, info, depth, last=False):
    ...
    d1 = d2 = depth//2
    if last:
        d1, d2 = _ensure_sum_divisible_by_6(d1, d2)
...
def YOLO_head(x, mode, params, info, grid_nn, cell_n):
    ...
    box_h = tf.nn.sigmoid(conv1x1_batch_norm(box_hr, mode, params, depth=cell_n))  # shape [batch, grid_nn, grid_nn, cell_n]
    ...
    info = _layer_stats(info, "YOLO head, box XYWHC", box_all, 1,
                        4 * _count_conv_weights(box_xr, box_x, 1) + _count_conv_weights(box_c, box_c_logits, 1))
